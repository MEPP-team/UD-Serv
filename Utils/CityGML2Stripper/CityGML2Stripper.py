...
def ParseCommandLine():
    ...
    parser.add_argument('--remove-empty-buildings',
                        action='store_true',
...
if __name__ == '__main__':
    ...
    if cli_args.remove_building_parts:
        ...
        ET.strip_elements(parsed_file,
                            '{' + parsed_file.getroot().nsmap['bldg'] + '}consistsOfBuildingPart' + '*')
    ...
    if cli_args.remove_empty_buildings:
        ...
        addBldgNamespace = lambda tag : '{' + parsed_file.getroot().nsmap['bldg'] + '}' + tag
        geometry_list = list(map(addBldgNamespace, geometry_list))
        for building in parsed_file.getroot().findall('.//{' + parsed_file.getroot().nsmap['bldg'] + '}Building'):
            has_geometry = False
            # if any Building children tags are in the geometry list, the Building is not considered empty
            ...
            if not cli_args.remove_building_parts:
                for building_part in building.findall('.//{' + parsed_file.getroot().nsmap['bldg'] + '}consistsOfBuildingPart'):
                    ...
                    if has_geometry:
                        break
            # if the building is considered empty, remove the cityObjectMember element that contains it
            if not has_geometry:
    ...
    parsed_file.write(cli_args.output,
